...
class doubleMax(objectFun_2):
    def __init__(self, population, allCoverRelation,prMat,topo_distance,nodeIndexId,distance_threshold=7):
        """
        @param population: 种群
        ...
        @param distance_threshold: 拓扑距离阈值
        """
    ...
    def objFun_1_Max(self):
        ...
        for indivi in self.population:
            ...
            monitored_ratio.append(monitored)
    ...
    def objFun_2_Max(self,pop_cover):
        ...
        pop_project=[]
        for indi_index in range(len(pop_cover_leak_data)):
        ...
        for indi_index in range(len(pop_project)):
            coverLeak_like_index=[np.where(row>0.95) for row in pop_project[indi_index]]
            cover_leak=pop_cover[indi_index]
            leak_like_dir=dict(zip(cover_leak,coverLeak_like_index))

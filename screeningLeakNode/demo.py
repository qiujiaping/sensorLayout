...
def getNode24hValue(inpFile):
    ...
    pressureIndex=[]
    demandIndex = []
    for junctionId in wn.junction_name_list:
        pressureIndex.append(pressure.loc[:,junctionId].values)
        demandIndex.append(demand.loc[:,junctionId].values)
    ...
    for node24hDem in demandIndex:
        # demandMax.append(np.max(node24hDem/(0.003785411784/60)))#换算单位结果为GPM
        # demandMax.append(np.max(node24hDem))    #不换算单位结果为si
        meanDeman.append(np.mean(node24hDem)/(0.003785411784/60))   #平均需求

    return pressureStd,pressureMax,meanDeman
# [0.27823573, 12.171179, 1.9208807, 0.98819224923831306, 314.92478274159384, 4.6441437418308258, 26.56135929152067]
...
def getNodeDegree(inpFile):
    ...
    degreeIndex=[]
    for junctionId in wn.junction_name_list:
        degreeIndex.append(G.degree(junctionId))
    return degreeIndex
...
def getNodeDiaAndLen(inpFile):
    ...
    for junction_name in wn.junction_name_list:
        node_link.update({junction_name:wn.get_links_for_node(junction_name)})
    node_linkLength={}
    node_linkDiameter={}
    # print(node_link)
    for key in node_link.keys():
        ...
        for pipId in wn.get_links_for_node(key):
            ...
            if(isinstance(wn.get_link(pipId),wntr.network.elements.Pipe)):
                # 记录与节点关联的每根管道的直径和长度(默认单位国际制)
                # diameter.append(wn.get_link(pipId).diameter*1000/25.4) #转为美制单位
                diameter.append(wn.get_link(pipId).diameter)
                # length.append(wn.get_link(pipId).length / 0.3048) #转为美制单位
                length.append(wn.get_link(pipId).length)
...
def getAllIndSME(inpFile):
    """
    获得节点压力均方差，最大压力，平均需求，节点的度数，关联的总管长，最小直径，关联的管段总体积7个指标
    """
    slic=1317
    IndicatorStd=[]  #存放每个指标的样本均方差
    pressureStd,pressureMax,meanDemand=getNode24hValue(inpFile)  #获得每个节点的压力均方差，最大压力，需求最大
    # print(demandMax)
    ...
    IndicatorStd.append(np.std(pressureStd[0:slic]))
    IndicatorStd.append(np.std(pressureMax[0:slic]))
    IndicatorStd.append(np.std(meanDemand[0:slic]))
    IndicatorStd.append(np.std(degreeIndex[0:slic]))
    IndicatorStd.append(np.std(node_totalLength[0:slic]))#
    # print(node_MinDiameter)
    IndicatorStd.append(np.std(node_MinDiameter[0:slic]))
    IndicatorStd.append(np.std(nodeTotalVolume[0:slic]))#
    wn=wntr.network.WaterNetworkModel(inpFile)
    return IndicatorStd

def drawPlot(inpFile):
    slice=1317
    pressureStd, pressureMax, meanDeman = getNode24hValue(inpFile)  # 获得每个节点的压力均方差，最大压力，平均需求
    degreeIndex = getNodeDegree(inpFile)  # 获得每个节点的度数
    node_totalLength, node_MinDiameter = getNodeToLenAndMinDia(inpFile)  # 获得节点关联的总管长，最小直径
    nodeTotalVolume = getNodeVolume(inpFile)  # 获得每个节点关联的管段总体积
    plt.subplot(331)
    plt.plot(list(range(slice)), pressureStd[0:slice], 'b-.')
    plt.title('pressureStd')
    plt.subplot(332)
    plt.plot(list(range(slice)), pressureMax[0:slice], 'b-.')
    plt.title('pressureMax')
    plt.subplot(333)
    plt.plot(list(range(slice)), meanDeman[0:slice], 'b-.')
    plt.title('meanDemand')
    plt.subplot(334)
    plt.plot(list(range(slice)), degreeIndex[0:slice], 'b-.')
    plt.title('degreeIndex')
    plt.subplot(335)
    plt.plot(list(range(slice)), node_totalLength[0:slice], 'b-.')
    plt.title('node_totalLength')
    plt.subplot(336)
    plt.plot(list(range(slice)), node_MinDiameter[0:slice], 'b-.')
    plt.title('node_MinDiameter')
    plt.subplot(337)
    plt.plot(list(range(slice)), nodeTotalVolume[0:slice], 'b-.')
...
def getAllData(inpFile):
    data=[]
    pressureStd, pressureMax, meanDeman = getNode24hValue(inpFile)  # 获得每个节点的压力均方差，最大压力，需求最大
    degreeIndex = getNodeDegree(inpFile)  # 获得每个节点的度数
    node_totalLength, node_MinDiameter = getNodeToLenAndMinDia(inpFile)  # 获得节点关联的总管长，最小直径
    nodeTotalVolume = getNodeVolume(inpFile)  # 获得每个节点关联的管段总体积
    data.append(pressureStd)
    data.append(pressureMax)
    data.append(meanDeman)
...
if __name__=="__main__":
    ...
    # drawPlot("data/ky8.inp")
    # drawPlot("data/ky8.inp")
    # data=getAllIndSME("data/ky8.inp")
    # print(data)
    # drawPlot("data/ky8.inp")
